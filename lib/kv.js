...
export async function trackDaily(name, countryCode) {
  const store = await getKV();
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  await store.hincrby(`daily:${today}`, name, 1);
  await store.hincrby(`daily:${today}`, "_total", 1);
  if (countryCode && countryCode !== "XX") {
    await store.hincrby(`daily:${today}:geo`, countryCode, 1);
  }
}
...
export async function getDailyStats(days = 14) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    const [data, geo, pvData, pvGeo] = await Promise.all([
      store.hgetall(`daily:${key}`),
      store.hgetall(`daily:${key}:geo`),
      store.hgetall(`pv:${key}`),
      store.hgetall(`pv:${key}:geo`),
    ]);
    const countries = geo
      ? Object.entries(geo).map(([code, count]) => ({ code, count: parseInt(count) })).sort((a, b) => b.count - a.count)
      : [];
    const pvCountries = pvGeo
      ? Object.entries(pvGeo).map(([code, count]) => ({ code, count: parseInt(count) })).sort((a, b) => b.count - a.count)
      : [];
    result.push({
      date: key,
      website: parseInt(data?.website || 0),
      ...
      pageviews: parseInt(pvData?._total || 0),
      countries,
      pvCountries,
    });
  }
...
export async function trackPageView(countryCode) {
  ...
  if (countryCode && countryCode !== "XX") {
    ...
    await store.hincrby(`pv:${today}:geo`, countryCode, 1);
  }
}
